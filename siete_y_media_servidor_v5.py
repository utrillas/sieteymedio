...
class Jugador(threading.Thread):
    ...
    def __init__(self, socket_jugador, addr_jugador, nombre_jugador, key_size, private_key, recipient_key):
        ...
        self.key_size, self.private_key, self.recipient_key = self.leer_clave()

    def leer_clave(self):
        try:    
            fichero_path_privada = Path(__file__).parent / "privada_servidor_sieteymedia.pem"
            with open(fichero_path_privada, 'r') as file:
                private_key = RSA.import_key(file.read())
            key_size = private_key.size_in_bytes()

            fichero_path_publica = Path(__file__).parent / "publica_usuarioA_sieteymedia.pem"
            with open(fichero_path_publica, 'rb') as file:
                recipient_key = RSA.import_key(file.read())

            return key_size, private_key, recipient_key
