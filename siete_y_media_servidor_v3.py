...
class Jugador(threading.Thread):
    ...
    def __init__(self, socket_jugador, addr_jugador, nombre_jugador, key_size, private_key, recipient_key):
        ...
        self.recipient_key = recipient_key
         ...
         # Generar y cargar claves privadas y públicas
        self.private_key, self.recipient_key, self.key_size = self.cargar_claves()

    def cargar_claves(self):
        try:
            # Cargar la clave privada del usuario
            fichero_privada = Path(__file__).parent / "privada_servidor_sieteymedia.pem"
            with open(fichero_privada, 'r') as file:
                private_key = RSA.import_key(file.read())
            key_size = private_key.size_in_bytes()

            # Cargar la clave pública del usuario
            fichero_publica = Path(__file__).parent / "publica_usuarioA_sieteymedia.pem"
            with open(fichero_publica, 'rb') as file:
                recipient_key = RSA.import_key(file.read())

            return private_key, recipient_key, key_size

        except Exception as e:
            print(f"Error al cargar las claves: {e}")
            return None, None, 0
    ...
    def recibir_info(self):
        ...
        msg_desenc = self.desencriptar_datos(msg_encript)
    ...
    def desencriptar_datos( self, msg_encript):
        try:
            #Separar los componentes cifrados del mensaje
            enc_session_key = msg_encript[:self.key_size]
            nonce = msg_encript[self.key_size:self.key_size+16]
            tag = msg_encript[self.key_size+16:self.key_size+32]
            cipher_text = msg_encript[self.key_size+32:]
...
if __name__ == "__main__":
    ...
    x = nombre_jugador()
    private_key, recipient_key, key_size = None, None, 0
    while True:
        #jugador aceptado
